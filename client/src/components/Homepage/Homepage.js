...
useEffect(() => {
  const fetchData = setInterval(async() => {
    // setLoading(true);
  const result = await axios.get("https://aithangouts-qir7.onrender.com/hangouts").then((res) => {
    console.log(res);
    // setLoading(false);
...
<Card className="galleryItem" style={{ width: '24rem'}}>
 ...
 {user_id === 'invalid' ? <>
 <FontAwesomeIcon className="utilIcon" icon={faHeart} size='lg' onClick={() => navigate('/login')}/>
